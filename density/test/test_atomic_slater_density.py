...
def test_slater_type_orbital_Be():
    ...
    expected = (2 * 0.821620)**2 * math.sqrt((2 * 0.821620)/ math.factorial(2 * 2)) * 2**1 * math.exp(-0.821620 * 2)
    assert abs(calculated - expected) < 1.e-6
    ...
    calculated = be.slator_type_orbital(Exp_Array, Quantum_Array, grid)
    expected1 = [(2 * 12.683501)**1 * math.sqrt((2 * 12.683501)/ math.factorial(2 * 1)) * 1**0 * math.exp(-12.683501 * 1),
                 (2 * 12.683501)**1 * math.sqrt((2 * 12.683501)/ math.factorial(2 * 1)) * 2**0 * math.exp(-12.683501 * 2)]
    expected2 = [(2 * 0.821620) **2 * math.sqrt((2 * 0.821620)/ math.factorial(2 * 2)) * (1**1) * math.exp(-0.821620 * 1),
                 (2 * 0.821620) **2 * math.sqrt((2 * 0.821620)/ math.factorial(2 * 2)) * (2**1) * math.exp(-0.821620 * 2)]
    expected = np.array([expected1, expected2]) #every row corresponds to one exponent evaluated on all grid points
...
def test_phi_LCAO_Be():
    ...
    LCAO1S = slater_function(12.683501, 1, r)*-0.0024917 + \
             ...
             slater_function(5.152556, 1, r)*0.0849694 + \
             slater_function(3.472467, 1, r)*0.8685562 + \
             slater_function(2.349757, 1, r)*0.0315855 + \
             slater_function(1.406429, 1, r)*-0.0035284 + \
             slater_function(0.821620, 2, r)*-0.0004149 + \
    ...
    LCAO2S = slater_function(12.683501, 1, r)*0.0004442 + \
             ...
             slater_function(5.152556, 1, r)*-0.0367056 + \
             slater_function(3.472467, 1, r)*0.0138910 + \
             slater_function(2.349757, 1, r)*-0.3598016 + \
             slater_function(1.406429, 1, r)*-0.2563459 + \
             slater_function(0.821620, 2, r)*0.2434108 + \
...
def test_phi_LCAO_Be_2():
    ...
    assert abs(phi[(0, 0)] - 0.38031668) < 1.e-6  #expected values are taken from the previous example
    assert abs(phi[(0, 1)] - 0.3278693) < 1.e-6   #expected values are taken from the previous example
...
def test_phi_matrix_Be():
    ...
    phi2S = slater_function(12.683501, 1, r) * 0.0004442  + \
            ...
            slater_function(0.821620, 2, r)  * 0.2434108  + \
            slater_function(0.786473, 1, r)  * 1.1150995
    expected = np.concatenate((np.array([phi1S]), np.array([phi2S])), axis=1)
...
def test_atomic_density_Be():
    ...
    # integrate(r^2 * density(r)) using the composite trapezoidal rule.
    assert abs(np.trapz(np.power(grid, 2) * density, grid) - 4.0) < 1.e-3
...
def test_atomic_density_Ne():
    ...
    # check: integrating atomic density should result in the number of electrons.
    # integrate(r^2 * density(r)) using the composite trapezoidal rule.
    assert abs(np.trapz(np.power(grid, 2) * density, grid) - 10.0) < 1.e-6
...
def test_atomic_density_C():
    ...
    # check: integrating atomic density should result in the number of electrons.
    # integrate(r^2 * density(r)) using the composite trapezoidal rule.
    assert abs(np.trapz(np.power(grid, 2) * density, grid) - 6.0) < 1.e-5
