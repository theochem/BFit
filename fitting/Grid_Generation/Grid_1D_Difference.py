...
class Grid_1D_Difference():
    ...
    def generate_difference_for_weights(self, index_of_grid):
        #TODO: I have no clue how he implemented this for difference index
        weights_for_1D_difference = np.empty(self.all_grids_obj.effort)

        grid_i_obj = self.all_grids_obj.all_1D_grids[index_of_grid]
        grid_i = grid_i_obj.grid_1D
        weight_i = grid_i_obj.weights_1D

        grid_i_min_1_obj = self.all_grids_obj.all_1D_grids[index_of_grid - 1]
        ...
        for index_i in range(0, grid_i_obj.num_of_points):
            ...
            for index_i_min_1 in range(0, grid_i_min_1_obj.num_of_points):
                if grid_i[index_i] == grid_i_min_1:
    ...
    def generate_different_for_indexes(self, index_of_grid):
        indexes_for_1D_difference = np.empty(self.all_grids_obj.effort)
        ...
        for index in range(0, grid_i_obj.num_of_points):
            for index2 in range(0, Grid_1D.max_num_of_pts_of_all_1D_grids):
                if (self.all_grids_obj.labels_of_all_1D_grids[index] == grid_i[index2]):
                    indexes_for_1D_difference[index2] = index
                    break
    ...
    def generate_all_differences(self):
        ############################# Weights #############################################
        self.all_weights_for_1D_difference[0] = copy.deepcopy(self.all_grids_obj.all_1D_grids[0].weights_1D)

        for x in range(2, self.all_grids_obj.effort + 1):
            self.all_weights_for_1D_difference[x] = self.generate_difference_for_weights(x)

        ############################# Indexes #############################################
        for x in range(1, self.all_grids_obj.effort + 1):
