...
class _GreedyStrategy(object):
    ...
    def run_greedy(self, factor, max_numb_of_funcs=30, backward_elim_funcs=None,
        ...
        if self.exit_info is None: #"Next Iteration Did Not Find THe Best Choice":
            self.exit_info = "Exited Because Number of functions is equal to maximum allowed functions " + \
                         ...
                         " or Cost function is less than some epsilon " + str(best_global_value - previous_global_value) + " <= " + str(1e-5) +\
    ...
    def run_greedy_initial(self, initial_params, factor, max_numb_of_funcs=30, backward_elim_funcs=None,
                   add_choice_funcs=None, ioutput=False):
        number_of_functions = len(initial_params)//2
        previous_global_value = 0
        best_global_value = 1e10
        storage_of_parameters_per_addition = [initial_params]
        number_of_redum = 0; initial_factor = factor
        while number_of_functions < max_numb_of_funcs - 1 and np.abs(best_global_value  - previous_global_value) >= 1e-5\
                and number_of_redum < 5:
            choices_of_parameters = self.get_next_iteration_of_variables(factor, global_parameters)
            ...
            number_of_functions = len(global_parameters) // 2
            coeffs, exps = check_redundancies(global_parameters[:len(global_parameters)//2],
                                              global_parameters[len(global_parameters)//2:])
            #global_parameters = np.append(coeffs, exps)
            if number_of_functions != len(coeffs): # Redudancies Found
...
class GreedyLeastSquares(_GreedyStrategy):
    def __init__(self, element_name, atomic_number, grid_obj, electron_density,
                 splitting_func=get_next_possible_coeffs_and_exps):
        self.atomic_number = atomic_number
        self.grid_obj = grid_obj
