...
class Atomic_Density():
    ...
    def get_hydrogen_wave_func(self, n=1, l=0, bohr_radius=1.):
        return np.power((1. / np.sqrt(np.pi) * (bohr_radius**(3./2.))) * np.exp(-self.row_grid / bohr_radius), 2.)
    ...
    def integrate_total_density_using_trapz(self):
        integrate = np.trapz(y=4. * np.pi * np.ravel(np.power(self.GRID, 2.0)) * np.ravel(self.electron_density), x=np.ravel(self.GRID) )
        return(integrate)
