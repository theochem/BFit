...
import numpy as np
import scipy
from fitting.mbis.GaussianBasisSet import *
...
def check_dimension_and_shape():
    ...
    assert np.shape(exponential)[0] == np.shape(self._grid)[0]
    ...
    assert np.shape(gaussian_density)[0] == np.shape(self._grid)[0]
...
def test_create_model():
    ...
    coefficient = 3.0
    exponent = 2.0
    exponential = np.exp(-1 * exponent * 1.0**2) * 3.0
...
def test_cost_function():
    ...
    assert np.abs(cost_function - manual_cost_function) < 1e-13
