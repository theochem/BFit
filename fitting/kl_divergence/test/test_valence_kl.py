...
def test_get_norm_constant():
    ...
    exponent = np.exp(-e * g.points**2.)
    ans = np.trapz(y=exponent * 4. * np.pi * g.points**2., x=g.points)
    ...
    ans = np.trapz(exponent * g.points**4. * 4. * np.pi, x=g.points)
    npt.assert_allclose(true_answer * ans, 1, rtol=1e-5)
...
def test_get_integration_factor_coeffs():
    ...
    desired_ans = trapz(integrand, x=g.points)
...
def test_updating_exponents():
    ...
    desired_answer /= (2. * simps(integrand2, g.points))

    integrand2 = np.ma.array(ratio * np.exp(-4 * g_s) * g_s**2.)
    desired_answer2 = 5. * simps(integrand2, g.points)
    integrand = np.ma.array(ratio * np.exp(-4 * g_s) * g_s * g_s * g_s)
    desired_answer2 /= (2. * simps(integrand, g.points))
