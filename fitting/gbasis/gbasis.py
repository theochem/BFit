...
class ContractedGaussianBasis:
    """
    A Contracted Gaussian Basis represented by a linear combination of gaussian
    basis primitives of the same type.
    """
    ...
    def __init__(self, shell_type, exponents, coefficients):
        """
            **Arguments:**
            ...
            shell_type
                A string of length one representing the type (quantum angular number)
                of the contracted gaussian basis. options: 's', 'p', 'd', 'f', 'g', 'h', 'i'

            exponents
                A 1D array containing the exponents of the primitives included in
                the contracted gaussian basis.

            coefficients
                A 1D array containing the coefficients of the primitives included
                in the contracted gaussian basis.

            It is possible to construct this object with lists instead of arrays.
            Just call to_arrays() method once the lists are completed. (This is convenient when
            loading a basis set from a file)
        """
        if not isinstance(shell_type, str):
            raise ValueError('shell_type should be a string. '
                             'type(sell_type)={0}'.format(type(shell_type)))
        if len(shell_type) != 1:
            raise ValueError('shell_type={0} should be a string of '
                             'length one.'.format(shell_type))
        if len(exponents) != len(coefficients):
            raise ValueError('exponents and coefficients should have the same length. '
                             '{0}!={1}'.format(len(exponents), len(coefficients)))
...
class UGBSBasis:
    ...
    def __init__(self, element):
        ...
        if element.lower() not in basis_atom_map.keys():
            raise ValueError('_element={0} does not exist in the basis set loaded '
                             'from {1}'.format(element, file_path))
        self.element = element.lower()
        self.basis = basis_atom_map[self.element]
        # Make sure that all contracted gaussian basis consist of only one primitive
    ...
    def exponents(self, shell_type='s'):
        ...
        """
        if not isinstance(shell_type, str) and len(shell_type) != 1:
