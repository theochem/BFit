...
from fitting.least_squares.least_sqs import optimize_using_nnls, \
    optimize_using_slsqp
import numpy as np
...
class GreedyLeastSquares(GreedyStrategy):
    ...
    def _solve_one_function_weight(self, weight):
        a = 2.0 * np.sum(weight)
        sum_of_grid_squared = np.sum(weight * np.power(self.grid.radii, 2))
        ...
        f = 2.0 * np.sum(weight * np.power(self.grid_obj, 2) *
    ...
    def get_errors_from_model(self, params):
        ...
        err1 = self.gauss_obj.integrate_model_trapz(params)
