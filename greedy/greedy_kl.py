...
class GreedyKL(GreedyStrategy):
    ...
    def get_optimization_routine(self, params, local=False):
        ...
        if local:
            return self.mbis_obj.__call__(0.1, 1e-2, coeff_arr, exp_arr, iprint=True)
        return self.mbis_obj.__call__(self.threshold_coeff, self.threshold_exp, coeff_arr, exp_arr, iprint=True)
