...
class ValenceMBIS(KullbackLeiblerFitting):
    def __init__(self, electron_density, grid_obj, weights, atomic_number, element_name):
        super(KullbackLeiblerFitting, self).__init__(electron_density, grid_obj, weights, atomic_number, element_name)
    ...
    def get_integration_factor_valence(self, exponent, masked_normed_gaussian, upt_exponents=False):
        ...
        if upt_exponents:
            return self.grid_obj.integrate_spher(normalized_ratio_weights * self.masked_grid_quadrupled * \
                                                 np.exp(-exponent * self.masked_grid_squared))
        return self.grid_obj.integrate_spher(normalized_ratio_weights *
    ...
    def run_valence(self, threshold_coeff, threshold_exp, coeff_arr, coeff2_arr, exp_arr, exp2_arr, iprint=False, iplot=False):
        ...
        storage_of_errors = [["""Integration Using Trapz"""],
                             [""" goodness of kl_divergence"""],
                             [""" goof of kl_divergence with r^2"""],
